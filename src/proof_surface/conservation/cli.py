"""`telos proof conservation` -- invariant-conservation receipt CLI.

Turns an input file (transformation + invariant + witnesses + negative_fixture)
into an artifact folder: packet.json, report.md, and the crucible thesis/
measurements. A packet whose negative fixture cannot break the invariant is
...
from .packet import validate_conservation_packet
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="telos proof conservation",
        description="Turn a conservation claim into an invariant-conservation proof packet.",
    )
    p.add_argument(
        "--input",
        required=True,
        help="JSON with transformation, invariant, witnesses, negative_fixture, sources",
    )
    ...
    p.add_argument("--packet-id", default="conservation", help="packet identifier")
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_receipts(out, domain="conservation", packet_id=args.packet_id)
    print(report)
    if issues:
