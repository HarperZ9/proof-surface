"""`telos proof optimization-workflow` -- optimization receipt CLI.

Turns an input file (sources + problem + candidate_space + baseline + solver) into
an artifact folder: packet.json, report.md, and the crucible thesis/measurements
for independent re-derivation of the solver-vs-baseline obligation. Stdlib-only.
"""
...
from .packet import validate_optimization_workflow_packet
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="telos proof optimization-workflow",
        description="Turn an optimization run into a solver-vs-baseline proof packet.",
    )
    p.add_argument(
        "--input",
        required=True,
        help="JSON with sources, problem, candidate_space, baseline, solver",
    )
    ...
    p.add_argument(
        "--packet-id", default="optimization-workflow", help="packet identifier"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    packet = build_optimization_workflow_packet(
        ...
        candidate_space=spec.get("candidate_space", {}),
    ...
    issues = validate_optimization_workflow_packet(packet)
    ...
    write_receipts(out, domain="optimization-workflow", packet_id=args.packet_id)
    print(report)
    if issues:
