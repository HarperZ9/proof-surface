"""`telos proof rollout-receipt` -- RL / post-training run receipt CLI.

Turns an input file (sources + rollout + reward + verifier + admission) into an
artifact folder: packet.json, report.md, and the crucible thesis/measurements.
Promotion is derived default-deny (promote only on a MATCH verifier + an allow
admission). Stdlib-only.
"""
...
from .packet import validate_rollout_receipt_packet
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="telos proof rollout-receipt",
        description="Turn a post-training run into a default-deny promotion proof packet.",
    )
    p.add_argument(
        "--input",
        required=True,
        help="JSON with sources, rollout, reward, verifier, admission",
    )
    ...
    p.add_argument("--packet-id", default="rollout-receipt", help="packet identifier")
...
def main(argv: list[str] | None = None) -> int:
    ...
    packet = build_rollout_receipt_packet(
    ...
    issues = validate_rollout_receipt_packet(packet)
    ...
    write_receipts(out, domain="rollout-receipt", packet_id=args.packet_id)
    print(report)
    if issues:
